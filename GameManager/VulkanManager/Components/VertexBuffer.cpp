...
void VertexBuffer::createVertexBuffer()
{
    ...
    // Gets the size of all the entities specified in the entity manager object    
    VkDeviceSize bufferSize = 0;    
    for (auto const& [key, value] : *pEntityManager->getPVertices())
    {
        bufferSize += sizeof(value[0][0]) * value[0].size();
    }
